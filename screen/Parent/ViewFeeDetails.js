...
const ViewFeeDetails = ({ navigation, route }) => {
    const { item } = route.params;
    const [feeInfo, setFeeInfo] = useState({});
    const getFeeDetails = async () => {
        try {
            // const user_name = await AsyncStorage.getItem('username');
            const user_name = item.reg_no;
    ...
    const renderButton = () => {
        if (feeInfo.status === "pending") {
            ...
            return (
                <View>
                    <Button mode="contained" style={styles.btn1} onPress={() => navigation.navigate("FeeStatus", { item })}>
                        View Fee Status
                    </Button>
                    <Button mode="contained" style={styles.btn1} onPress={() => navigation.navigate("FeeChallans", { item })}>
                        View Challan
                    </Button>
                </View>
