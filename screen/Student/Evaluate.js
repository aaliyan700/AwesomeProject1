...
const Evaluate = ({ navigation, route }) => {
    ...
    const setValue = (questionId, answer) => {
        ...
        if (answers[questionId] === answer) {
            ...
            // Remove the question from myArray
            setMyArray((prevArray) => prevArray.filter((item) => item.questionId !== questionId));
        } else {
            // If it is not, update the selected answer in the answers object
            setAnswers((prevAnswers) => ({ ...prevAnswers, [questionId]: answer }));
        ...
        <ScrollView>
            ...
            {question?.map((ele, index) => {
                if (ele != null) {
                    ...
                    return (
                        <View key={index} style={styles.questionDiv}>
                            <Text style={{ color: 'black', fontSize: 15 }}>{`Q${index + 1}: ${ele.question}`}</Text>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginTop: 5 }}>
                                <TouchableOpacity
                                    style={[styles.btnStyle, selectedAnswer === 4 && styles.selectedBtnStyle]}
                                    onPress={() => setValue(ele.id, 4)}>
            ...
            <Button
                mode="contained"
                color="#099e78"
                style={{ marginHorizontal: 40, margin: 10 }}
                onPress={() => submit()}>
                Submit
            </Button>
        </ScrollView>
